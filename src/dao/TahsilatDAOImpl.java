...
public class TahsilatDAOImpl implements TahsilatDAO {
	...
	@Override
	@Transactional
	public List<Tahsilat> hayvanNoyaGöreHissedargetir(Long hayvanNo) {
		Session session = sessionFactory.getCurrentSession();
		Query sorgu = session
				.createQuery("from Tahsilat where hayvanNo =:x order by odemezamani desc");
		sorgu.setParameter("x", hayvanNo); // sorgu.setParameter("x",
													// "%" + id +// "%"); SELECT
													// hayvan_no,
													// COUNT(hayvan_no) FROM
													// buyukbas Group By
